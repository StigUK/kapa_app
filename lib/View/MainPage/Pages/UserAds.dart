...
class _UserAdsPageState extends State<UserAdsPage> {
              ...
              child: Padding(
                ...
                child: TabBar(
                  ...
                  tabs: [
                    Tab(
                        child: Container(
                            child: Text("Активні", style: defaultTextStyle))),
                    Tab(
                        child: Container(
                            child: Text("Архів", style: defaultTextStyle))),
                  ],
                ),
              ),
            ),
    ...
    return loadData
        ? Container(
            child: listArchiveAds.length > 0
                ...
                : Center(
                    child: Text(
                    "У вас поки немає архівованих оголошень",
                    style: defaultTextStyle,
                    textAlign: TextAlign.center,
                  )))
        : Center(child: CircularProgressIndicator());
  ...
  loadAds() async {
    ...
    await _db
        .collection("adsArchive")
        .getDocuments()
        .then((QuerySnapshot snapshot) {
      snapshot.documents.forEach((f) {
        Ad tempAd = Ad.fromDocument(f);
        if (user.uid == tempAd.userId) listArchiveAds.add(tempAd);
      });
    });
    await _db.collection("ads").getDocuments().then((QuerySnapshot snapshot) {
      snapshot.documents.forEach((f) {
        Ad tempAd = Ad.fromDocument(f);
        if (user.uid == tempAd.userId) listActiveAds.add(tempAd);
      });
      if (this.mounted)
        setState(() {
